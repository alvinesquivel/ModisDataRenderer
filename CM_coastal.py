...
img = plt.imshow(np.flipud(sds_data_0_bin), vmin = 0, vmax = 1, cmap = cmap, interpolation = 'nearest',
	origin = 'lower')
#cbar = plt.colorbar(img, cmap = cmap, norm = norm, boundaries = bounds, ticks = bounds)
#cbar.ax.set_yticklabels(['Water', 'Coastal'], fontsize=10)
ax.set_xlabel("", fontsize=7)
ax.set_ylabel("", fontsize=1)



plt.title('Cloud Mask - Surface Type (Coastal Area)', fontsize = 10)
plt.show()
